  ...
  if (bman.notifygamemaster)
    {
      ...
      if (ogc_init (bman.ogc_port, host, port, "BomberClone",
                    bman.net_ai_family) == 0)
...
network_loop ()
{
   ...
   */
  if (bman.state == GS_wait || bman.state == GS_ready
...
net_send_playerid (int pl_nr)
{
  ...
  if ((players[pl_nr].gfx_nr >= 0
       && players[pl_nr].gfx != &gfx.players[players[pl_nr].gfx_nr])
...
send_ogc_update ()
{
  ...
  ogc_sendgamestatus (bman.sock, "BomberClone", VERSION, bman.gamename, j,
                      bman.maxplayer,
                      status);
};
