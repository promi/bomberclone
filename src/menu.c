...
menu_draw_background (_menu * menu, SDL_Rect * updaterect)
{
  ...
  for (; y <= (menu->oldscreenpos.h - 2 * menuimages[0]->h - 1) /
       menuimages[4]->h; y++)
    {
      x = 0;                  // start at the updaterect. start pos
      for (; x <= (menu->oldscreenpos.w - 2 * menuimages[0]->w - 1) /
           menuimages[4]->w; x++)
        {
          ...
          if (dx >= (menu->oldscreenpos.w - 2 * menuimages[0]->w))
            dest.w = menuimages[4]->w - (dx - (menu->oldscreenpos.w - 2 *
                                               menuimages[0]->w));
          else
          ...
          if (dy >= (menu->oldscreenpos.h - 2 * menuimages[0]->h))
            dest.h = menuimages[4]->h - (dy - (menu->oldscreenpos.h - 2 *
                                               menuimages[0]->h));
          else
...
menu_draw_border (_menu * menu)
{
  ...
  // draw top and below
  for (i = 0;
       i < ((menu->oldscreenpos.w - (2 * menuimages[0]->w)) / menuimages[1]->w); i++)
    {
  ...
  // draw left and right
  for (i = 0;
       i < ((menu->oldscreenpos.h - (2 * menuimages[0]->h)) / menuimages[3]->h); i++)
    {
      dest.x = menu->oldscreenpos.x;
  ...
  // draw title
  dx = menu->oldscreenpos.x + (menu->oldscreenpos.w - font[2].size.x * strlen (
                                 menu->title)) / 2;
  font_gfxdrawbold (dx, menu->oldscreenpos.y + menuimages[0]->h + 8, menu->title,
                    2, COLOR_brown,
                    2, 10000);
  font_gfxdraw (dx, menu->oldscreenpos.y + menuimages[0]->h + 8, menu->title, 2,
                COLOR_yellow,
                10000);
};
...
int
menu_create_dirlist (char *path, signed char dirflags, _charlist * cl,
                     int maxentry)
{
  int cnt;
  _direntry *destart,
...
void
menu_formattext (char *input, char *out, char **start, int *lines,
                 int *maxlinelen, int max_chars,
                 int max_lines)
{
  int i,
      pos,
  ...
  *maxlinelen = i = pos = outpos = *lines = 0;
  ...
  do
    {
      if (input[i] ==
          ' ')    // new word check if there is still space left for another word
        {
  ...
  while (i < strlen (input) && i < max_lines * (max_chars + 1)
         && *lines < max_lines);
};
