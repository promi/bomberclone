...
chat_findfreeline ()
{
  ...
  if (i >= CHAT_MAX_LINES-1)
    {
      memcpy (&chat.lines[0], &chat.lines[1],
              sizeof (chat.lines[1]) * (CHAT_MAX_LINES - 1));
      i = CHAT_MAX_LINES - 2;
    }
  else
    chat.curline++;
...
chat_drawlines ()
{
  int vislines = (chat.window.h - 20) /
                 font[0].size.y; // number of  visible lines
  ...
  for (nr = 0; i <= chat.curline; i++, nr++)
    font_gfxdraw (chat.window.x + 4, chat.window.y + 4 + font[0].size.y * nr,
                  chat.lines[i].text, 0, chat.lines[i].color, 0x1000);
};
...
chat_drawinput ()
{
  ...
  font_gfxdraw (chat.window.x + 2,
                (chat.window.y + chat.window.h) - (2 + font[0].size.y), chat.input.text, 0,
                COLOR_black, 0x1000);
