...
int keybinput_loop (_keybinput *ki, SDL_Event *event)
{
  ...
  if (event->type == SDL_KEYDOWN && keybinput_oldkey != event->key.keysym.sym)
    {
      ...
      else if (ki->type == KEYBI_text && ((keyu >= 32 && keyu <= 126) || (keyu >= 128
                                          && keyu <= 255)))
        {
      ...
      else if (ki->type == KEYBI_int && (keyu == '-' || (keyu >= '0'
                                         && keyu <= '9')))
        {
      ...
      else if (ki->type == KEYBI_float && (keyu == '-' || keyu == '.' || (keyu >= '0'
                                           && keyu <= '9')))
        {
  ...
  if ((keybinput_oldkey == SDLK_RETURN || keybinput_oldkey == SDLK_KP_ENTER)
