  ...
  while (menuselect != -1 && bman.state != GS_quit)
    {
      ...
      switch (map.map_selection)
        ...
        case (MAPS_randgen):   // Random Generated Map
          ...
          d_printf ("autogen\n");
          menu_create_entry (menu, "X Size:", 20, 160, 120, &map.size.x, MAX_FIELDSIZE_X,
                             MENU_entryint16, 2);
          menu_create_entry (menu, "Y Size:", 20, 180, 120, &map.size.y, MAX_FIELDSIZE_Y,
                             MENU_entryint16, 3);
          break;
      ...
      menu_create_entry (menu, "Bombs:", 140, 240, 110, &map.bombs, 100,
                         MENU_entryint32, 7);
      menu_create_entry (menu, "Fire: ", 140, 260, 110, &map.fire, 100,
                         MENU_entryint32, 8);
      menu_create_entry (menu, "Shoes:", 140, 280, 110, &map.shoes, 100,
                         MENU_entryint32, 9);
      menu_create_entry (menu, "Mixed:", 140, 300, 110, &map.mixed, 100,
                         MENU_entryint32, 10);
      menu_create_entry (menu, "Death:", 140, 320, 110, &map.death, 100,
                         MENU_entryint32, 11);

      menu_create_entry (menu, "Trigger:", 280, 240, 120, &map.sp_trigger, 30,
                         MENU_entryint32, 12);
      menu_create_entry (menu, "Push:   ", 280, 260, 120, &map.sp_push, 30,
                         MENU_entryint32, 13);
      menu_create_entry (menu, "Row:    ", 280, 280, 120, &map.sp_row, 30,
                         MENU_entryint32, 14);
      menu_create_entry (menu, "Kick:   ", 280, 300, 120, &map.sp_kick, 30,
                         MENU_entryint32, 15);
  ...
  while (menuselect != -1 && bman.state != GS_quit)
    {
      ...
      menu_create_entry (menu, "Bombs:", 25, 80, 150, &bman.start_bombs, MAX_BOMBS,
                         MENU_entryint32, 1);
      menu_create_entry (menu, "Speed:", 25,110, 150, &bman.start_speed, MAX_SPEED,
                         MENU_entryfloat, 2);
      menu_create_entry (menu, "Range:", 25,140, 150, &bman.start_range, MAX_RANGE,
                         MENU_entryint32, 3);

      menu_create_label (menu, "Other Values", 200, 50, 0, COLOR_brown);
      menu_create_entry (menu, "Gametime:", 200, 80, 150, &bman.init_timeout, 1200,
                         MENU_entryint32, 4);

      menu_create_label (menu, "Game Parameter", 25, 180, 1, COLOR_brown);
      menu_create_entry (menu, "Bomb Time:", 25, 220, 200, &bman.bomb_tickingtime,
                         1200, MENU_entryfloat, 5);
      menu_create_label (menu, "Gametype", 25, 250, 0, COLOR_brown);
      menu_create_list (menu, "GameType", 25, 280, 150, 60, gametype, &gametype_sel,
                        6);
