...
gfx_blitdraw ()
{
  ...
  for (i = 0; i < MAX_BLITRECTS && sortblitdb[i] != NULL; i++)
    {
      SDL_BlitSurface (sortblitdb[i]->srci, &sortblitdb[i]->srcr,
                       sortblitdb[i]->desti, &sortblitdb[i]->destr);
...
/* Add a new image to draw/blit on the screen and in thw right order.
   ...
   y:           deep of the image */
void
gfx_blit (SDL_Surface *srci, SDL_Rect *srcr, SDL_Surface *desti,
          SDL_Rect *destr, int y)
{
  int i;
