...
void rscache_loop ()
{
  ...
  if (bman.state==GS_running) timeout = RESENDCACHE_TIMEOUT;
  else timeout=RESENDCACHE_TIMEOUT_MENU;
      ...
      if (timestamp - rscache.entry[i].timestamp >= timeout
          && rscache.entry[i].retry < RESENDCACHE_RETRY)
        {
          ...
          ("Data Send Timeout Resend pl:%p, typ:%u id:%u Fill:%d Pos:%d\n",
           rscache.entry[i].addr.pl_nr, rscache.entry[i].packet.h.typ,
           rscache.entry[i].packet.h.id, rscache.count, i);
          ...
          if (rscache.entry[i].addr.pl_nr >= 0
              && rscache.entry[i].addr.pl_nr < MAX_PLAYERS)
