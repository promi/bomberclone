...
static int ogc_listadd (char *data)
{
  ...
  for (i = 0; (i < MAX_OGC_ENTRYS && ogc_array[i].serial != -1
               && ogc_array[i].serial != ge.serial); i++);
...
int ogc_init (char *localport, char *server, char *port, char *game,
              int ai_family)
{
  int i;
  ...
  d_printf ("ogc_host:%s ogc_port:%s ogc_game:%s ogc_ai_family:%d\n", ogc_host,
            ogc_port, ogc_game, ogc_ai_family);
  if (dns_filladdr (ogc_host, LEN_OGCHOST, ogc_port, LEN_OGCPORT, ogc_ai_family,
                    &ogc_addr) < 0)
    {
      udp_close (ogc_sock);
      ogc_sock = -1;
