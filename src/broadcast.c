...
void broadcast_del (int nr)
{
  ...
  if (nr < (BC_MAXENTRYS -1))
    memcpy (&broadcast_list[nr], &broadcast_list[nr+1],
            sizeof (struct broadcast_entry) * (BC_MAXENTRYS - (nr+1)));
...
void broadcast_check ()
{
  ...
  for (i = 0; i < BC_MAXENTRYS; i++)
    {
      ...
      if (((broadcast_list[i].timestamp < 0)
           || (timestamp - broadcast_list[i].timestamp < 0))
  ...
  for (i = 0; i < MAX_OGC_ENTRYS; i++)
    if ((ogc_array[i].serial != -1)
        && (broadcast_find (ogc_array[i].host, ogc_array[i].port) == -1))
      {
...
void broadcast_loop ()
{
  ...
  inlen = udp_get (bman.sock, indata, MAX_UDPDATA, &addr.sAddr,
                   bman.net_ai_family);
  if (inlen > 0)
    dns_filladdr (addr.host, LEN_SERVERNAME, addr.port, LEN_PORT,
                  bman.net_ai_family,
