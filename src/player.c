  ...
  else if (PS_IS_respawn (player->state))
    {
      ...
      * first: draw the star */
      ...
      if ((int)player->frame < gfx.respawn.frames)
        ...
        src.y = (2 * gfx.block.y) * (int)(gfx.respawn.frames - (((
                                            int)player->frame) - gfx.respawn.frames - 1));
  ...
  if (p->m == 1)
    {
      ...
      if (p->d == left && _pos.y == 0.0f
          && ((_pos.x == 0.0f && check_field (p->pos.x - 1.0f, p->pos.y))
              || (_pos.x > 0.0f)))
        d.x = -speed;
      ...
      if (p->d == up && _pos.x == 0.0f
          && ((_pos.y == 0.0f && check_field (p->pos.x, p->pos.y - 1.0f))
              || (_pos.y > 0.0f)))
        d.y = -speed;
      ...
      if (_pos.x == 0.0f && _pos.y == 0.0f
          && map.field[(int)p->pos.x][(int)p->pos.y].type == FT_tunnel
          && p->tunnelto <= 0.0f)
        {
...
get_killer_for_explosion (short int x, short int y)
{
  ...
  for (i = 0; i < 4; i++)
    if (map.field[x][y].ex[i].count > 0)
      {
        if (players[map.field[x][y].ex[i].bomb_p].bombs[map.field[x][y].ex[i].bomb_b].ex_nr
            == map.field[x][y].ex_nr )
...
player_move (int pl_nr)
{
  ...
  if (p->tunnelto > 0.0f)
  ...
  else
    {
      ...
      if (PS_IS_alife (p->state) && (CUTINT(p->pos.x) > EXPLOSION_SAVE_DISTANCE
                                     && (p->d == left || p->d == right))
          && (!check_exfield (p->pos.x + 1.0f, p->pos.y)))
        player_died (p, get_killer_for_explosion(p->pos.x + 1.0f, p->pos.y), 0);
      else if (PS_IS_alife (p->state) && (CUTINT(p->pos.y) > EXPLOSION_SAVE_DISTANCE
                                          && (p->d == up || p->d == down))
               && (!check_exfield (p->pos.x, p->pos.y + 1.0f)))
        player_died (p, get_killer_for_explosion(p->pos.x, p->pos.y + 1.0f), 0);
      else if (PS_IS_alife (p->state)
               && ((CUTINT(p->pos.x) < (1.0f - EXPLOSION_SAVE_DISTANCE) && (p->d == left
                    || p->d == right))
...
{
  if ((player - players) != bman.p_nr && (player - players) != bman.p2_nr
      && PS_IS_netplayer (player->state) && network == 0)
    return;

  // player die !
  d_printf ("player_died net:%d pl_nr:%d dead_by_nr:%d - %-10s\n", network,
            player - players, dead_by, players[dead_by].name);
      ...
      if (player - players != dead_by)
        {
          ...
          if (bman.gametype == GT_team && players[dead_by].team_nr >= 0
              && players[dead_by].team_nr < MAX_TEAMS)
  ...
  if ((GT_SP || player == &players[bman.p_nr] || (IS_LPLAYER2
       && player == &players[bman.p2_nr])
...
player_ilness_loop (int plnr)
{
  ...
  for (pnr = 0; pnr < MAX_PLAYERS; pnr++)
    if (pnr != plnr && PS_IS_alife (players[pnr].state)
        && PS_IS_netplayer (players[pnr].state))
      {
        p = &players[pnr];
...
void player_checkdeath (int pnr)
{
  ...
  if (bman.gametype == GT_deathmatch && PS_IS_respawn (player->state)
      && player->frame >= 2*gfx.respawn.frames)
    {
...
player_delete (int pl_nr)
{
  ...
  if (pl_nr == bman.p_nr)
  ...
  else
    {
      ...
      players[pl_nr].state &= (0xFF - (PSF_used + PSF_net + PSF_alife +
                                       PSF_ai)); /* delete
												player flags */
...
void player2_join ()
{
  ...
  if (IS_LPLAYER2)
    {
      menu_displaymessage ("Sorry",
                           "Sorry there is already a second player from this computer.");
      return;
    }
