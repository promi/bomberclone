...
void do_joingame (struct pkg_joingame *p_jg, _net_addr * addr)
{
  ...
  if (bman.passwordenabled
      && (strncmp (bman.password, p_jg->password, LEN_PASSWORD)
...
do_playerid (struct pkg_playerid *p_id, _net_addr * addr)
{
  ...
  ("do_playerid (From:%s:%s pl_nr=%d) Player(name:%s [%s:%s], pl_nr:%d state:%d,%d)\n",
   addr->host, addr->port, addr->pl_nr, p_id->name, p_id->host, p_id->port,
   p_id->pl_nr, p_id->state, PS_IS_alife (p_id->state));

  /*
   * check if we have to send the whole playerlist to a client
   */
  if (GT_MP_PTPM && p_id->pl_nr == -1 && addr->pl_nr >= 0
      && addr->pl_nr < MAX_PLAYERS)
    {
   ...
   */
           ...
           || (GT_MP_PTPS && p_id->pl_nr >= 0 && p_id->pl_nr < MAX_PLAYERS))
    {
      ...
      if (p_id->host[0] != 0
          && p_id->pl_nr != bman.p_servnr)   // copy addr only if p_id != server
        {
       ...
       * to set it's host and port name */
               ...
               && p_id->pl_nr != bman.p_servnr)   // copy addr only if p_id != server
        {
          strncpy (pl->net.addr.host, players[bman.p_servnr].net.addr.host,
                   LEN_SERVERNAME);
      ...
      if (GT_MP_PTPS && !(PS_IS_netplayer (pl->state))
          && (PS_IS_netplayer (p_id->state))
      ...
      if (GT_MP_PTPM && GS_WAITRUNNING && addr->pl_nr >= 0
          && addr->pl_nr < MAX_PLAYERS)
...
do_servermode (struct pkg_servermode *s_mod, _net_addr * addr)
{
  ...
  if ((!s_mod->lplayer2 && bman.p_nr == -1) || (s_mod->lplayer2
      && bman.p2_nr == -1))
    {
  ...
  if (f_dat->x < map.size.x && f_dat->y < map.size.y)
    {
      ...
      map.field[f_dat->x][f_dat->y].type =
        f_dat->type; // CHECK_BFIELD::: FOR BOMBS:::::
...
send_ping (_net_addr * addr, int data, unsigned char typ)
{
  ...
  players[addr->pl_nr].net.timestamp =
    timestamp; /* we need to set it here, so we can check
...
do_playerdata (struct pkg_playerdata *p_dat, _net_addr * addr)
{
  ...
  d_printf ("do_playerdata (From: %d) p_nr: %d --> state alife? %d new %d\n",
            addr->pl_nr, p_dat->p_nr, PS_IS_alife(players[p_dat->p_nr].state) ,
            PS_IS_alife(p_dat->state));
  ...
  if (p_dat->dead_by < 0
      || p_dat->dead_by >= MAX_PLAYERS) /* set player state no matter what */
    pl->state = p_dat->state;
...
void do_bombdata (struct pkg_bombdata *b_dat, _net_addr * addr)
{
  ...
  d_printf ("do_bombdata [%f,%f] Player: %d PlayerIgnition: %d Bomb: %d, ex_nr:%d\n",
            ...
            b_dat->pi_nr, b_dat->b_nr, NTOH32 (b_dat->ex_nr));
  ...
  if (bomb->state != BS_off)   // handle push & kick special
    {
      map.bfield[(int) bomb->pos.x][(int) bomb->pos.y] =
        0; //remove bomb at old location
  ...
  if (bomb->state == BS_off && (b_dat->state == BS_ticking
                                || b_dat->state == BS_trigger))
  ...
  if (bomb->state != b_dat->state && bomb->state != BS_ticking)
    bomb->to = I32TOF (NTOH32 (
                         b_dat->to)); /* only set if the bomb isn't already ticking
                                                   to make sure the timeout won't be resetted
                                                   by an old network packet */

  map.bfield[(int) bomb->pos.x][(int) bomb->pos.y] =
    1; // keep the bfield up to date

  bomb->r = b_dat->r;
  bomb->ex_nr = NTOH32 (b_dat->ex_nr);
  bomb->state = b_dat->state & 0x0F;

  if (bomb->mode != b_dat->state >>
      4)  		// bombmode changed set source to current position
    {
...
send_dropitems (_net_addr * addr, int pl_nr, _flyingitem ** fitems, int cnt)
{
  ...
  struct pkg_dropitem *dipkg = (struct pkg_dropitem *)
                               outdata; // set the pointer to outdata
  int i;

  dipkg->h.typ = PKG_dropitem;
  dipkg->h.len = HTON16 (sizeof (struct pkg_dropitem) + cnt * sizeof (
...
send_mapinfo (_net_addr * addr)
{
  ...
  snprintf (map_pkg.start_speed, sizeof(map_pkg.start_speed), "%f",
            bman.start_speed);
  snprintf (map_pkg.bomb_tickingtime, sizeof(map_pkg.bomb_tickingtime), "%f",
...
send_gameinfo (_net_addr * addr, int sock, int broadcast)
{
  ...
  if (broadcast)
    ...
    udp_send (sock, (char *) &pgi, NTOH16 (pgi.h.len), &addr->sAddr,
              bman.net_ai_family);
};
...
send_pkg (struct pkg *packet, _net_addr * addr)
{
  ...
  udp_send (bman.sock, (char *) packet, NTOH16 (packet->h.len), &addr->sAddr,
            bman.net_ai_family);
