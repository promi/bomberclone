...
void draw_bomb (_bomb * bomb)
{
  ...
  if (bomb->pos.x < 0 || bomb->pos.y < 0 || bomb->pos.x >= map.size.x
      || bomb->pos.y >= map.size.y)
    {
      d_printf ("FATAL: Draw Bomb out of range [%f,%f]\n", bomb->pos.x, bomb->pos.y);
      return;
    }

  if (bomb->state != BS_trigger || ((bomb->state == BS_trigger)
...
void bomb_explode (_bomb *bomb, int net)
{
  ...
  d_printf ("Bomb Explode p:%d, b:%d, pI:%d, exe_nr:%d [%f,%f]\n", bomb->id.p,
            bomb->id.b, bomb->id.pIgnition, bomb->ex_nr, bomb->pos.x, bomb->pos.y);
  ...
  if (GT_MP_PTPM
      && net)      /* from now on only the server let the bomb explode */
...
void explosion_restore (_bomb *bomb)
{
  ...
  if (bomb->mode == BM_moving || bomb->mode == BM_pushed
      || bomb->mode == BM_liquid)
    map.bfield[(int) bomb->pos.x + (int)bomb->dest.x][(int) bomb->pos.y +
        (int)bomb->dest.y] = 0;
              ...
              while (bomb->firemaxr[d] == 0 && (bomb->firer[d] < new_range))
                {
                  ...
                  if (ftype == FT_nothing || ftype == FT_tunnel)
                    {
                      if (map.field[dx][dy].ex[d].count == 0
  ...
  for (i = 0; pl[i] != -1; i++)
    {
      ...
      if (((tmpplayer->state & PSF_alife) != 0)
          && (GT_SP
              || (GT_MP && (&players[bman.p_nr] == tmpplayer || (IS_LPLAYER2
                            && &players[bman.p2_nr] == tmpplayer)))
...
void bomb_kicked (_bomb * bomb)
{
  ...
  if (bomb->fdata >= 1.0f)
  ...
  else
    {
      ...
      bomb->pos.y = (bomb->source.y + dY * bomb->fdata) - (4.0-(4.0 * powf ((
                      -0.5+bomb->fdata)*2.0f, 2.0f)));
    }
};
