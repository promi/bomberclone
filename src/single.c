...
void ai_team_choosegfx ()
{
   ...
   */
  for (tm_nr = 0; tm_nr < MAX_TEAMS; tm_nr++) for (pl_nr = 0; pl_nr < MAX_PLAYERS;
        pl_nr++)
      {
...
ai_bombpoints (_point pos, int range)
{
  ...
  if (pos.x < 0 || pos.x >= MAX_FIELDSIZE_X || pos.y < 0
      || pos.y >= MAX_FIELDSIZE_Y)
    {
  ...
  if (map.field[pos.x][pos.y].type != FT_block
      && map.field[pos.x][pos.y].type != FT_stone
      ...
      for (d = 0; d < 4; d++)
        {
          ...
          if ((p.x >= 0 && p.x < map.size.x && p.y >= 0 && p.y < map.size.y)
              && map.field[p.x][p.y].type != FT_nothing
              && map.field[p.x][p.y].type != FT_tunnel
                ...
                if (nearbomb == 0)   // no bombs found
                ...
                else
                  {
                    ...
                    if (rawdir.dir != 0 && rawdir.bestdir == -1)
                      {
                        pl->d = ai_choosedir (rawdir.dir, nearbomb,
                                              pl->d); // we have to make a choice.. do it
                        pl->m = 1;
                      }
...
single_menu ()
{
  ...
  menu_create_text (menu, "numpl", 20, 50, 12, 2, COLOR_yellow,
                    "Number of\nAI Players");
  menu_create_list (menu, "AI Players", 40, 90, 50, 100, nrplayerlist,
                    &selnrplayer, 3);
