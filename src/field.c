...
static float
fieldcheck_to;     // timeout for the next fieldcheck FIELDCHECK_TIMEOUT
...
stonelist_draw ()
{
  ...
  for (i = 0; i < MAX_STONESTODRAW && stonelist[i].x != -1
       && stonelist[i].y != -1; i++)
    {
...
_stonelist_add (int x, int y, int recursive)
{
  ...
  if (slentry == NULL)        // no space left
  ...
  else
    {
      ...
      if (recursive
          && gfx.field[map.field[x][y].type].h != gfx.field[map.field[x][y].type].w)
...
draw_stone (int x, int y)
{
  ...
  if (y < map.size.y - 1
      && gfx.field[map.field[x][y + 1].type].h > gfx.field[map.field[x][y +
          1].type].w)
    {
      src.x = 0;
      ...
      gfx_blit (gfx.field[map.field[x][y + 1].type].image, &src, gfx.screen, &dest,
                (y*256) + 5);
    }
...
int field_check_alldirs (int x, int y, int type)
{
  if (x <= 0 || y <= 0 || x >= map.size.x-1 || y >= map.size.y-1)
    return 0;
  if (map.field[x-1][y].type == type || map.field[x+1][y].type == type
      || map.field[x][y-1].type == type || map.field[x][y+1].type == type)
    return 1;
  else
    return 0;
}
  ...
  for (i = 0; i < MAX_FIELDANIMATION; i++)
    if (fieldani[i].x >= 0 && fieldani[i].x < map.size.x && fieldani[i].y >= 0
        && fieldani[i].y < map.size.y)
      {
        ...
        if ((stone->type == FT_stone && stone->frame > 0.0f)
            || (stone->type >= FT_death))
          {
            /* animate this stone */
    ...
    if (stone->ex[d].count > 0)
      {
        ...
        if (stone->ex[d].bomb_p >= 0 && stone->ex[d].bomb_p < MAX_PLAYERS
            && stone->ex[d].bomb_b >= 0 && stone->ex[d].bomb_b < MAX_BOMBS)
          {
            ...
            if (players[stone->ex[d].bomb_p].bombs[stone->ex[d].bomb_b].state !=
                BS_exploding)
              {
                stone->ex[d].count--;
                stone->ex[d].bomb_b = -1;
