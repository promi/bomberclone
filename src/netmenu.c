  ...
  while (menuselect != -1 && !done && bman.state != GS_quit)
    {
      ...
      menu_create_list (menu, "Players", 150, 55, 55, 70, nrplayerlist, &selnrplayer,
                        1);
      menu_create_entry (menu, "Gamename", -1, 150, 300, &bman.gamename, LEN_GAMENAME,
                         MENU_entrytext, 3);
      menu_create_entry (menu, "UDP Port:", -1, 180, 200, &bman.port, LEN_PORT,
                         MENU_entrytext,4);
#ifndef _WIN32
      menu_create_bool (menu, "IPV6", 260, 60, 100, &net_ai_typ, 2);
#endif
      menu_create_bool (menu, "Private Game", 20, 220, 150, &bman.passwordenabled, 6);
      menu_create_entry (menu, "Password:", 190, 220, 210, &bman.password,
                         LEN_PASSWORD, MENU_entrytext, 7);
      ...
      menu_create_entry (menu, "OpenGameCache:", -1, 280, 350, &bman.ogcserver,
                         LEN_SERVERNAME, MENU_entrytext, 10);
      menu_create_entry (menu, "Local OGC Port", -1, 310, 350, &bman.ogc_port,
                         LEN_PORT, MENU_entrytext, 11);
  ...
  if (bman.servername[0] != 0)
    {
      ...
      dns_filladdr (players[0].net.addr.host, LEN_SERVERNAME,
                    players[0].net.addr.port,
