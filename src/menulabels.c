...
void menu_draw_label (_menuitem *mi)
{
  ...
  if (mi->pos.x == -1)
    dx = (menu->oldscreenpos.w - 2*menuimages[0]->w - (strlen (
            mi->label) * font[mi->pos.w].size.x)) / 2;
  else
    dx = mi->pos.x;
  if (mi->pos.y == -1)
  ...
  font_gfxdraw (menu->oldscreenpos.x + menuimages[0]->w + dx,
                menu->oldscreenpos.y + menuimages[0]->h + dy, mi->label, mi->pos.w, mi->pos.h,
                10000);
};


_menuitem *menu_create_label (_menu *menu, char *name, int x, int y,
 ...
 * any pointer
 */
void menu_create_text (_menu *menu, char *name, int x, int y, int maxlen,
