  ...
  for (i = 0; i < 3; i++)
    {
      ...
      SDL_SetColorKey (tmp , SDL_SRCCOLORKEY, SDL_MapRGB (tmp->format, 255, 255,
                       255));
      ...
      for (c = 0; c < COLOR_max; c++)
        {
          ...
          SDL_SetColorKey (font[i].image[c] , SDL_SRCCOLORKEY,
                           SDL_MapRGB (font[i].image[c]->format, 0,0,0));
          SDL_FillRect (font[i].image[c], NULL, SDL_MapRGB (font[i].image[c]->format, r,g,
                        b));
...
void font_gfxdrawbold (int x, int y, char *text, int size, int color, int bold,
                       int ypos)
{
