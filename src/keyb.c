...
void keyb_code2text (int keycode, char *keytext)
{
  ...
  for (i = 0; (key_codetab [i].code != -1
               && key_codetab[i].code != keycode); i++);
 ...
 */
void keyb_config_createkeymenu (_menu *menu, int key, int x, int y,
                                int menu_nr)
{
  int key_id;
  char keyname [32];
  ...
  for (key_id = key; key_id >= BCPK_max
       && key_id < BCPK_max * 3; key_id = key_id - BCPK_max);
...
void keyb_config_joypad (int key)
{
  ...
  menu_displaytext ("Joypad Config",
...
void keyb_config_readkey (int key)
{
  ...
  menu_displaytext ("Keyboard Config",
                    "Please press the new key\nfor this function.");
  timestamp = SDL_GetTicks (); // needed for time sync.
      ...
      if ( joy[1] != NULL )
        {
          ...
          menu_create_button (menu, text, 225 + 70, 250, 100,
                              10 + BCPK_max + BCPK_max + BCPK_drop);
          ...
          menu_create_button (menu, text, 225 + 70, 280, 100,
                              10 + BCPK_max + BCPK_max + BCPK_special);
        }
  ...
  do
    {
      ...
      keyb_config_createkeymenu (menu, BCPK_max + BCPK_max + BCPK_right, 225, 220,
                                 10);
      keyb_config_createkeymenu (menu, BCPK_max + BCPK_max + BCPK_drop, 225, 250, 10);
      keyb_config_createkeymenu (menu, BCPK_max + BCPK_max + BCPK_special, 225, 280,
                                 10);
