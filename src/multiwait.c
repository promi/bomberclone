...
static void mw_keys_loop ()
{
  if (GT_MP_PTPM && mw_check_players () >= 2 && keyb_gamekeys.state[BCK_pause]
  ...
  if (GT_MP_PTPM && mw_check_players () < 2 && keyb_gamekeys.state[BCK_pause]
      && !keyb_gamekeys.old[BCK_pause])
    {
      font_gfxdraw (20,20,"There are not enough configured players", 0, COLOR_brown,
                    0x1000);
      font_gfxdraw (20,40,"(each player needs to press Ctrl to select the role)", 0,
                    COLOR_brown, 0x1000);
    }
  ...
  if (((IS_LPLAYER2) && keyb_gamekeys.state[BCPK_max + BCPK_drop]
       && !keyb_gamekeys.old[BCPK_max + BCPK_drop])
      || ((!IS_LPLAYER2) && keyb_gamekeys.state[BCPK_drop]
          && !keyb_gamekeys.old[BCPK_drop]))
    {
  ...
  if (IS_LPLAYER2 && keyb_gamekeys.state[BCPK_max + BCPK_max + BCPK_drop]
      && !keyb_gamekeys.old[BCPK_max + BCPK_max + BCPK_drop])
    {
  ...
  if (keyb_gamekeys.state[BCK_mapmenu] && !keyb_gamekeys.old[BCK_mapmenu]
      && GT_MP_PTPM)
    {
      /* mapmenu */
      mapmenu ();
