  ...
  for (; flitem != NULL; flitem = flitem->next)
    {
      ...
      if (flitem->type == FT_nothing || flitem->step >= 1.0f)
        ...
        {
          flitem->pos.x = (1.0f - flitem->step) * (flitem->from.x - flitem->to.x) +
                          flitem->to.x;
          flitem->pos.y = (1.0f - flitem->step) * (flitem->from.y - flitem->to.y) +
                          flitem->to.y;
          flitems_draw (flitem);
          old = &flitem->next;
        }
  ...
  do
    {
      ...
      check = (to.x > 0 && to.y > 0 && to.x < map.size.x-1 && to.y < map.size.y-1
               ...
               && (map.field[to.x][to.y].type == FT_nothing
                   || (map.field[to.x][to.y].type == FT_stone
                       && map.field[to.x][to.y].special == FT_nothing))
...
void flitems_dropitems (int p_nr, _pointf from, int cnt_speed, int cnt_bombs,
                        int cnt_range)
{
  int i, lpos = 0;
  _point to;
...
void flitems_draw (_flyingitem *flitem)
{
  ...
  if (flitem == NULL || (flitem->type != FT_shoe && flitem->type != FT_fire
                         && flitem->type != FT_bomb))
    return;
  ...
  gfx_blit (gfx.powerup[0].image, &src, gfx.screen, &dest,
            (((int)flitems->pos.y + 1.0)*256) + 255);
  gfx_blit (srci, &src, gfx.screen, &dest,
            (((int)flitems->pos.y + 1.0)*256) + 256);
};
