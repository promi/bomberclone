  ...
  for (i = 0; i < MAX_PLAYERS; i++)
    {
      if (players[i].gamestats.isaplayer == 1 )
        {
          sprintf(text, "%02d %-16s   %2d       %d  ", i, players[i].name,
                  players[i].gamestats.killed, players[i].gamestats.unknown);
...
void d_playerdetail (char *head)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
    ...
    d_printf ("%2d %16s %3d %3d %3d %3d %3d %4d   %3d     %p[%s:%s]\n",i,
              players[i].name, players[i].gfx_nr, players[i].state, players[i].points,
              players[i].wins, players[i].nbrKilled, players[i].team_nr, players[i].net.flags,
              players[i].net.addr.host, &players[i].net.addr, players[i].net.addr.port);
};
...
void d_teamdetail (char *head)
{
  ...
  d_printf ("Teams: | %-10s | %-10s | %-10s | %-10s | Players\n", teams[0].name,
            teams[1].name,teams[2].name,teams[3].name);
  for (p = 0; p < MAX_PLAYERS; p++)
    {
      ...
      d_printf ("       | %-10s | %-10s | %-10s | %-10s | %-10s Team:%d\n", name[0],
                name[1], name[2], name[3], players[p].name, players[p].team_nr);
    }
};
