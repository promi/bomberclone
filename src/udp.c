...
extern int dns_filladdr (char *host, int hostlen, char *port, int portlen,
                         int ai_family,
                         struct _sockaddr *sAddr);
extern int udp_get (int sock, char *text, int len, struct _sockaddr *sAddr,
                    int ai_family);
extern int udp_server (char *port, int ai_family);
extern void udp_send (int sock, char *text, int len, struct _sockaddr *sAddr,
                      int ai_family);
              ...
              struct _sockaddr *sAddr)
{
  ...
  if (host[0] == 0 || port[0] == 0)
  ...
  else
    {
      ...
      ((struct sockaddr_in *) sAddr)->sin_port = htons (atoi (
            port)); // short, network byte order
      ((struct sockaddr_in *) sAddr)->sin_addr = *((struct in_addr *) he->h_addr);
      memset (&(((struct sockaddr_in *) sAddr)->sin_zero), '\0',
...
void udp_sendbroadcast (int sock, char *text, int len, struct _sockaddr *sAddr,
                        int ai_family)
{
  char value;
...
udp_get (int sock, char *text, int len, struct _sockaddr *sAddr, int ai_family)
{
  ...
  tval.tv_usec = 100;
  msglen = 0;
  if (select (sock + 1, &sockset, NULL, NULL, &tval)) if (FD_ISSET (sock,
        &sockset))
      {
        msglen = recvfrom (sock, text, len, MSG_DONTWAIT, (struct sockaddr *) sAddr,
                           &clen);
        if (msglen < 0)
          return 0;
...
dns_net_getip (char *host)
{
  ...
  if (hAddr == NULL)
    return NULL;
  strncpy (dnsip, inet_ntoa (*((struct in_addr *) hAddr->h_addr)),
           UDP_LEN_HOSTNAME);
