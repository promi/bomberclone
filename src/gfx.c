...
gfx_load_players (int sx, int sy)
{
  ...
  for (i = 0; i < gfx.player_gfx_count; i++)
    {
      ...
      if (tmpimage == NULL)
      ...
      else
        {
          ...
          tmpimage1 = scale_image (tmpimage, gfx.players[i].ani.w * 4,
                                   gfx.players[i].ani.frames * gfx.players[i].ani.h);
          getRGBpixel (tmpimage1, 0, 0, &r, &g, &b);
          SDL_SetColorKey (tmpimage1, SDL_SRCCOLORKEY, SDL_MapRGB (tmpimage1->format, r,
                           g, b));
  ...
  tmpimage1 = scale_image (tmpimage, ((2 * sx * tmpimage->w) / (2 * GFX_IMGSIZE)),
  ...
  gfx.respawn.image  = scale_image (tmpimage,
                                    (2 * sx * tmpimage->w) / (2 * GFX_IMGSIZE), gfx.respawn.frames * (2 * sy));
  SDL_FreeSurface (tmpimage);
};
...
gfx_loaddata ()
{
  ...
  SDL_FreeSurface (tmpimage);
  SDL_SetColorKey (tmpimage1, SDL_SRCCOLORKEY, SDL_MapRGB (tmpimage1->format, 255,
                   255, 0));
    ...
    for (i = 0; i < 3; i++)
      {
        ...
        SDL_SetColorKey (tmpimage, SDL_SRCCOLORKEY, SDL_MapRGB (tmpimage->format, 255,
                         255, 255));
...
static void gfx_load_menusmall_players ()
{
      ...
      if (gfx.players[i].small_image == NULL || gfx.players[i].menu_image == NULL)
        {
          ...
          if (gfx.players[i].small_image == NULL)
            {
              tmpimg = scale_image (img, (int)(((float)img->w)*sfkt),
                                    GFX_SMALLPLAYERIMGSIZE_X * 2);
  ...
  gfx.ghost = scale_image (orgimg, (int)(((float)orgimg->w)*sfkt),
                           GFX_MENUPLAYERIMGSIZE_X * 2);
  if (gfx.ghost == NULL)
    {
      printf ("Can't load image: %s\n", SDL_GetError ());
  ...
  gfx.ghost_small = scale_image (orgimg, (int)(((float)orgimg->w)*sfkt),
                                 GFX_SMALLPLAYERIMGSIZE_X * 2);
  ...
  for (ft = 0; ft < FT_max; ft++) if (ft != FT_mixed)
      {
        ...
        sprintf (filename, "%s/tileset/default/%s.png", bman.datapath,
                 ft_filenames[ft]);
