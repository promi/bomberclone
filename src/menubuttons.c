/* $Id: menubuttons.c,v 1.3 2015/07/07 20:57:06 steffen Exp $
 ...
 * is called */
_menuitem *menu_create_button (_menu *menu, char *name, int x, int y, int w,
                               int id)
{
  ...
  m->type = MENU_button;
  m->pos.w = (1 + (int)((w - menubuttonimages[0][0]->w -
                         menubuttonimages[0][2]->w) / menubuttonimages[0][1]->w)) *
             menubuttonimages[0][1]->w + menubuttonimages[0][0]->w +
             menubuttonimages[0][2]->w;
  if (x != -1)
    m->pos.x = x;
  else
...
void menu_draw_button (_menuitem *mi)
{
  ...
  for (i = 0;
       i < ((mi->pos.w - (menubuttonimages[mi->state][0]->w +
                          menubuttonimages[mi->state][2]->w)) / menubuttonimages[mi->state][1]->w); i++)
    {
      dest.x = MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x +
               menubuttonimages[mi->state][0]->w + (i * menubuttonimages[mi->state][1]->w);
      dest.y = MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y;
  ...
  px = (mi->pos.w - (strlen (mi->label) * font[MENU_BUTTON_FONTSIZE].size.x)) / 2
       + mi->pos.x;
  py = (menubuttonimages[mi->state][0]->h - font[MENU_BUTTON_FONTSIZE].size.y) / 2
       + mi->pos.y;
