...
game_draw_info ()
{
  ...
  if (GT_MP && (chat.oldscreen == NULL || chat.window.x != 4
                || chat.window.y != 4.5 * 16))
    {
  ...
  if (bman.updatestatusbar)
    {
        ...
        if ((players[i].state & PSFM_used) != 0)
          {
            ...
            sprintf (scrtext, "%10s:%d %1d %1d", players[i].name, players[i].wins,
                     players[i].points, players[i].nbrKilled);
      ...
      if (bman.state == GS_ready && GT_MP_PTPM)
        ...
        font_gfxdraw (100, 32, "Waiting for the Server to Start", 0, COLOR_yellow,
                      0xFFFF);
  ...
  if (map.state != MS_normal)
    {
      ...
      if (bman.updatestatusbar || hurrywarn_to <= 0.0
  ...
  if (bman.p_nr >= 0 && bman.p_nr < MAX_PLAYERS)
    {
      ...
      if (GT_MP_PTPS)
        send_playerdata (&players[bman.p_servnr].net.addr, bman.p_nr,
                         &players[bman.p_nr]);
    }
  if (bman.p2_nr >= 0 && bman.p2_nr < MAX_PLAYERS)
    {
      ...
      if (GT_MP_PTPS)
        send_playerdata (&players[bman.p_servnr].net.addr, bman.p2_nr,
                         &players[bman.p2_nr]);
    }
...
game_showresultteam (int pos_x, int pos_y, int pos_w, int pos_h)
{
  ...
  for (t_nr = 0; t_nr < MAX_TEAMS; t_nr++)
    {
      for (p_nr = 0; (p_nr < MAX_PLAYERS
                      && teams[t_nr].players[p_nr] == NULL); p_nr++);
  ...
  for (t_nr = 0; t_nr < t_count; t_nr++)
    {
      sprintf (text, "%s Victorys %d (%d)", tdata[t_nr].team->name,
               tdata[t_nr].team->wins,
      ...
      for (col = 0, p_nr = 0; p_nr < tdata[t_nr].cnt; p_nr++)
        {
          ...
          sprintf (text, "%s(%d/%d)", tdata[t_nr].pl[p_nr]->name,
                   tdata[t_nr].pl[p_nr]->wins,
                   tdata[t_nr].pl[p_nr]->points);
          font_gfxdraw (10 + pos_x + x + GFX_SMALLPLAYERIMGSIZE_X * 2, pos_y + sy + 2,
                        text, 0, 0,
                        2);
          x += dx;
          col++;
      ...
      if (PS_IS_used (pl[p]->state))
        {
          if (PS_IS_alife (pl[p]->state))
            ...
            font_gfxdraw (10 + pos_x + x + GFX_MENUPLAYERIMGSIZE_X, pos_y + y - 10,
                          pl[p]->name,
                          0, COLOR_gray, 1);

          sprintf (text, "%3d (%2d)", pl[p]->points, pl[p]->wins);
          font_gfxdraw (10 + pos_x + x + GFX_MENUPLAYERIMGSIZE_X, pos_y + y + 6, text, 0,
                        0, 1);
...
game_showresult ()
{
  ...
  font_drawbold (x, gfx.res.y - (2 * font[0].size.y) - 2, text, 0, COLOR_brown,
                 1);
  ...
  while (!done && bman.state != GS_quit)
    {
      ...
      if ((keys[SDLK_RETURN] || keys[SDLK_KP_ENTER] || keys[SDLK_LCTRL]
           || keys[SDLK_RCTRL]) && (!keypressed)
