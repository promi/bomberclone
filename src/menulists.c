 ...
 * is called */
_menuitem *menu_create_list (_menu *menu, char *name, int x, int y, int w,
                             int h, _charlist *data, _charlist **selected, int id)
{
  ...
  menuitems->pos.w = (1 + (int)
                      ((w - menulistimages[0][0]->w - menulistimages[0][2]->w) /
                       menulistimages[0][1]->w))
                     * menulistimages[0][1]->w + menulistimages[0][0]->w + menulistimages[0][2]->w;
  menuitems->pos.h = (1 + (int)
                      ((h - menulistimages[0][0]->h - menulistimages[0][6]->h) /
                       menulistimages[0][3]->h))
                     * menulistimages[0][3]->h + menulistimages[0][0]->h + menulistimages[0][6]->h;
  if (x != -1)
    menuitems->pos.x = x;
  else
    menuitems->pos.x = (menu->oldscreenpos.w - 2 * menuimages[0]->w -
                        menuitems->pos.w) / 2;
  menuitems->pos.y = y;
...
void menu_draw_listbackground (_menuitem *mi, SDL_Rect *updaterect)
{
  ...
  for (; y <= (mi->pos.h - 2*menulistimages[0][0]->h - 1)/menulistimages[0][4]->h;
       y++)
    {
      x = 0; // start at the updaterect. start pos
      for (; x <= (mi->pos.w - 2*menulistimages[0][0]->w - 1)/menulistimages[0][4]->w;
           x++)
        {
          ...
          if (dx >= (mi->pos.w - 2*menulistimages[0][0]->w))
            dest.w = menulistimages[0][4]->w - (dx - (mi->pos.w -
                                                2*menulistimages[0][0]->w));
          else
            dest.w = menulistimages[0][4]->w;

          dy = (1+y) * menulistimages[0][4]->h;
          if (dy >= (mi->pos.h - 2*menulistimages[0][0]->h))
            dest.h = menulistimages[0][4]->h - (dy - (mi->pos.h -
                                                2*menulistimages[0][0]->h));
          else
          ...
          if (dest.w > 0 || dest.h > 0)
            {
              ...
              if (updaterect == NULL)
                ...
                {
                  window = *updaterect;
                  window.x += MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x +
                              menulistimages[0][0]->w;
                  window.y += MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y +
                              menulistimages[0][0]->h;
...
void menu_draw_listtext (_menuitem *mi)
{
  ...
  int countvis = (mi->pos.h - menulistimages[0][0]->h - menulistimages[0][6]->h) /
  ...
  maxx = (mi->pos.w - menulistimages[0][0]->w - menulistimages[0][2]->w) /
         font[MENU_BUTTON_FONTSIZE].size.x;

  /* calculate start point (y) */
  dy = MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y + ((mi->pos.h -
       countvis*font[MENU_BUTTON_FONTSIZE].size.y) / 2);
  ...
  for (; countvis > 0 && list != NULL; countvis--, list = list->next)
    {
      ...
      text[maxx] = 0;
      dx = MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x + ((mi->pos.w - strlen (
             text)*font[MENU_BUTTON_FONTSIZE].size.x) / 2);

      if (mi->ptrdata != NULL && list == *(_charlist **)mi->ptrdata)
        {
          // this is the selected element
          wnd.x = menulistimages[0][0]->w;
          wnd.y = dy - (MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y +
                        menulistimages[0][0]->h);
...
void menu_draw_list (_menuitem *mi)
{
  ...
  dest.x = MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x + mi->pos.w -
           menulistimages[focus][2]->w;
  gfx_blit (menulistimages[focus][2], NULL, gfx.screen, &dest, 10000);
  // draw the bottom left and right of the list
  dest.y = MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y + mi->pos.h -
           menulistimages[focus][8]->h;
    ...
    case (SDL_KEYDOWN): /* key was pressed */
      ...
      if (event->key.keysym.sym == SDLK_LEFT)
        ...
        menu_list_select_next (mi);
               ...
               || event->key.keysym.sym == SDLK_RCTRL || event->key.keysym.sym == SDLK_RCTRL)
        return 1;
      break;
    case (SDL_KEYUP):
