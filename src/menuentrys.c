/* $Id: menuentrys.c,v 1.5 2015/07/07 20:57:06 steffen Exp $
 * Menuhandling: entry */
 ...
 * is called */
_menuitem *menu_create_entry (_menu *menu, char *name, int x, int y, int w,
                              void *data, int len, int typ, int id)
{
  _menuitem *m = menuitem_findfree (menu);
  if (m == NULL) return NULL;

  m->type = typ;
  m->pos.w = (1 + (int)((w - (strlen (name) * font[MENU_BUTTON_FONTSIZE].size.x) -
                         8 - menuentryimages[0][0]->w - menuentryimages[0][2]->w) /
                        menuentryimages[0][1]->w)) * menuentryimages[0][1]->w + menuentryimages[0][0]->w
             + menuentryimages[0][2]->w + (strlen (name) * font[MENU_BUTTON_FONTSIZE].size.x)
             + 8;
...
void menu_draw_entry (_menuitem *mi)
{
  ...
  if (mi->type != MENU_entrytext && mi->type != MENU_entryint16
      && mi->type != MENU_entryint32 && mi->type != MENU_entryfloat)
    return;
  ...
  dest.x = MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x + (strlen (
             mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) + 8;
  ...
  if (mi->pos.w < ((strlen (mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) + 8)
      - (menuentryimages[mi->state][0]->w + menuentryimages[mi->state][2]->w))
    d_fatal ("menuentry Error with Element: %s\n", mi->label);
  else
    for (i = 0;
         i < ((mi->pos.w - ((strlen (mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) + 8)
               - (menuentryimages[mi->state][0]->w + menuentryimages[mi->state][2]->w)) /
              menuentryimages[mi->state][1]->w); i++)
      {
        dest.x = MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x + (strlen (
                   mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) + 8 +
                 menuentryimages[mi->state][0]->w + (i * menuentryimages[mi->state][1]->w);
        dest.y = MENUOFFSET_Y(((_menu *)mi->menu)) + mi->pos.y;
  ...
  px = (strlen (mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) + 8 +
       (mi->pos.w - (strlen (mi->label) * font[MENU_BUTTON_FONTSIZE].size.x) - 8 -
        (strlen (mi->keybi.text) * font[MENU_BUTTON_FONTSIZE].size.x)) / 2 + mi->pos.x;
  py = (menuentryimages[mi->state][0]->h - font[MENU_BUTTON_FONTSIZE].size.y) / 2
       + mi->pos.y;
  font_gfxdraw (MENUOFFSET_X(((_menu *)mi->menu)) + px,
                MENUOFFSET_Y(((_menu *)mi->menu)) + py, mi->keybi.text, MENU_BUTTON_FONTSIZE,
                COLOR_yellow, 10000);
  font_gfxdraw (MENUOFFSET_X(((_menu *)mi->menu)) + mi->pos.x,
                MENUOFFSET_Y(((_menu *)mi->menu)) + py, mi->label, MENU_BUTTON_FONTSIZE,
                COLOR_yellow, 10000);
};
