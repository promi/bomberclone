...
map_fillitems (int fieldtype, int num)
{
  ...
  for (d = 0; d < num * fkt; d++)
    {
      x = y = 0;
      while (map.field[x][y].type != FT_stone
...
map_new (char *filename)
{
  ...
  map_find_and_add_start_points(pl_cnt - map_num_defined_start_points(),
                                MAP_POSITION_TOLERENCE);
...
{
  if ((map_num_defined_start_points() < MAX_PLAYERS)
      && (map_check_start_point(x, y, tol)))
    {
