...
void srvlist_rebuildlist ()
{
        ...
        if (broadcast_list[i].host[0] != 0 && broadcast_list[i].lan == j)
          {
            ...
            strncpy (srvlst_dat[srvlst_cnt].version, broadcast_list[i].version,
                     LEN_VERSION);
            strncpy (srvlst_dat[srvlst_cnt].gamename, broadcast_list[i].gamename,
                     LEN_GAMENAME);
  ...
  if (bman.notifygamemaster)
    {
      for (ogclst = 0; (ogclst < MAX_OGC_ENTRYS
                        && srvlst_cnt < MAX_SRVLIST); ogclst++)
        if ((ogc_array[ogclst].serial != -1)
            && (broadcast_find (ogc_array[ogclst].host, ogc_array[ogclst].port) == -1))
          {
            ...
            strncpy (srvlst_dat[srvlst_cnt].gamename,ogc_array[ogclst].gamename,
                     LEN_GAMENAME);
  ...
  for (i = 0; i < srvlst_cnt; i++)
    {
      ...
      if (srvlst_dat[i].gamename[0] != 0) /* gamename is present */
        sprintf (txt1, "%-15s %-5s %5s  %-11s %-4s", srvlst_dat[i].gamename, txt3, txt2,
                 srvlst_dat[i].version, srvlst_dat[i].comment);
      else
        {
          sprintf (txt1, "%s:%s", srvlst_dat[i].host, srvlst_dat[i].port);
          sprintf (txt1, "%-20s %5s  %-11s %-4s", txt1,txt2, srvlst_dat[i].version,
...
void net_getserver ()
{
  int menuselect = 0, entry = 0, newentry = 0, eventstate, done,
      srvlst_lastrebuild = 0;
  ...
  srvlst_listmenu = menu_create_list (menu, "Host a Game", -1, 50, 475, 250,
                                      srvlst_text, &sel_entry, 1);
  menu_create_entry (menu, "IP :", -1, 320, 475, bman.servername,
                     LEN_SERVERNAME+LEN_PORT + 2, MENU_entrytext, 2);
  ...
  while (menuselect != -1 && bman.state != GS_quit)
    {
      ...
      if ((newentry != entry || (bman.servername[0] == 0
                                 && srvlst_dat[entry].host[0] != 0)) && menu->focus->id != 2)
        {
          entry = newentry;
          d_printf ("Selected Entry (%d) %s:%s Game:%s\n", entry, srvlst_dat[entry].host,
                    srvlst_dat[entry].port, srvlst_dat[entry].gamename);
          if (srvlst_dat[entry].host[0] != 0
              ...
              && srvlst_dat[entry].gamename[0] != 0)    /* test if there was a selection */
            {
              ...
              sprintf (bman.servername, "%s:%s", srvlst_dat[entry].host,
                       srvlst_dat[entry].port);
            }
          srvlst_listmenu->changed = 1;
          menu_reload (menu);
          menu_draw (menu);
