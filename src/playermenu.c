      ...
      for (i = 0; i < i_end - i_start; i++)
        {
          ...
          rect.x = (2 * GFX_MENUPLAYERIMGSIZE_X) * (i % 4) + ((menu->oldscreenpos.w - 2 *
                   menuimages[0]->w) - (8 * GFX_MENUPLAYERIMGSIZE_X)) / 2;
          ...
          gfx_blit (gfx.players[i + i_start].menu_image, &srcrect, gfx.screen, &rect,
                    10002);
          ...
          if ((i + i_start) == selgfx)
            {
              ...
              rect.x = GFX_IMGSIZE * (i % 4) + ((menu->oldscreenpos.w - 2 * menuimages[0]->w)
                                                -(4 * GFX_IMGSIZE)) / 2;
              rect.y = GFX_IMGSIZE * (i / 4) + PLAYERMENU_GFXSEL_Y;
              rect.x += (gfx.players[i + i_start].menu_image->w - gfx.menuselect.image->w) /
                        2; // center the playergfx
              rect.x += menu->oldscreenpos.x + menuimages[0]->w;
              rect.y += menu->oldscreenpos.y + menuimages[0]->h;
...
int playermenu_selgfx (int pl_nr)
{
  ...
  menu_create_text (menu, "playergfxsel", -1, 50, 40, 5, COLOR_yellow,
                    "%s, please select your Player and press ENTER/RETURN or press ESCAPE for no player (that means you will only watch the next game).",
                    players[pl_nr].name);
          ...
          switch (event.type)
            ...
            case (SDL_KEYDOWN):
               ...
               */
                       ...
                       || event.key.keysym.sym == keyb_gamekeys.keycode[BCPK_drop]
                       || event.key.keysym.sym == keyb_gamekeys.keycode[BCPK_special]
                       || event.key.keysym.sym == keyb_gamekeys.keycode[BCPK_max + BCPK_drop]
                       || event.key.keysym.sym == keyb_gamekeys.keycode[BCPK_max + BCPK_special])
                {
                  done = 1;
                  break;
                }
...
void playermenu ()
{
  ...
  list_PlayerList = menu_create_list (menu, "playerlist", 15, 70, 170, 160,
                                      playerlist, &playerlist_sel, 6);

  menu_create_label (menu, "Details", 240, 50, 0, COLOR_brown);

  detail_Name = menu_create_label (menu, "Name", 220, 100, 0, COLOR_yellow);
  detail_Addr = menu_create_label (menu, "10.10.10.1:6666", 220, 120, 0,
                                   COLOR_yellow);
  ...
  do
    {
              ...
              if (i == (playerlist_sel - &playerlist[0]))
                {
                  if (players[pl_nr].team_nr >= 0 && players[pl_nr].team_nr < MAX_TEAMS)
                    sprintf (detail_Name->label, "%s(%s)", players[pl_nr].name,
                             teams[players[pl_nr].team_nr].name);
                  else
                    sprintf (detail_Name->label, "%s", players[pl_nr].name);
                  sprintf (detail_Addr->label, "%-32s:%s",players[pl_nr].net.addr.host,
                           players[pl_nr].net.addr.port);
       ...
       */
      if (done == 1
          && menu->focus->id == btn_SecondPlayer->id)  	/* second local player want to join */
        {
          if (IS_LPLAYER2)
  ...
  for (i = 0; i < MAX_TEAMS; i++)
    {
      ...
      menu_create_entry (menu, "Name :", 25, y, 200, teams[i].name, LEN_PLAYERNAME,
                         MENU_entrytext, 2*i + 1);
      menu_create_label (menu, "Color:", 250, y, 0, COLOR_yellow);
      menu_create_list (menu, "Liste", 320, y, 90, 20, tdata[i].collist,
                        &tdata[i].collistsel, 2*i + 2);
...
void playernamemenu ()
{
  ...
  menu_create_entry (menu, "Player 1:", -1, 50, 300, &bman.playername,
                     LEN_PLAYERNAME, MENU_entrytext, 1);
  menu_create_entry (menu, "Player 2:", -1, 80, 300, &bman.player2name,
                     LEN_PLAYERNAME, MENU_entrytext, 2);
